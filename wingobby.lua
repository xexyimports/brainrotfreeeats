--[[
    xexy hub
    Game: 73768006371986
    Features: Auto Train • Auto Win • Auto Rebirth
]]
...
-- Auto Train (fast but not laggy)
task.spawn(function()
    while true do
        if AutoTrain then
            local event = ReplicatedStorage:FindFirstChild("Events") and ReplicatedStorage.Events:FindFirstChild("AddSpeed")
            if event then
                pcall(function()
                    event:FireServer()
                end)
            end
            task.wait(0.03) -- ~33 times per second — fast + smooth
        else
            task.wait(0.15)
        end
    end
...
RunService.Heartbeat:Connect(function()
    ...
    if AutoRebirth then
        local event = ReplicatedStorage:FindFirstChild("Events") and ReplicatedStorage.Events:FindFirstChild("RequestRebirth")
        if event then
            pcall(function()
                event:InvokeServer()
            end)
...
print("[xexy hub] loaded — Auto Train / Win / Rebirth ready")
