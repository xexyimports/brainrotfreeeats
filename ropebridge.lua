...
task.spawn(function()
    ...
    IsGameSupported = SupportedPlaceIds[CurrentGameId] == true

    -- Resolve names
    for id in pairs(SupportedPlaceIds) do
        local ok, info = pcall(function()
            return MarketplaceService:GetProductInfo(id)
        end)
        if ok and info and info.Name then
            SupportedGamesInfo[id] = info.Name
        else
            SupportedGamesInfo[id] = "Place " .. tostring(id)
        end
...
local function getPlayerPlot()
    ...
    for _, plot in ipairs(plotsFolder:GetChildren()) do
        if plot:FindFirstChild("Pods") then
            local success, plotPos = pcall(function()
                return plot:GetPivot().Position
            end)
            if success then
                local dist = (plotPos - root.Position).Magnitude
...
local function startAutoMoney()
    ...
    moneyThread = task.spawn(function()
        while autoMoney do
            ...
            local pods = plot and plot:FindFirstChild("Pods")
...
-- Supported Games List + Teleport buttons
local GamesHeader = makeLabel(MainPage, "SUPPORTED GAMES (click to join)", 242)

local GamesContainer = Instance.new("Frame")
GamesContainer.Name = "GamesContainer"
GamesContainer.Size = UDim2.new(1, -16, 0, 300)
...
local function buildFarmPage()
    for _, child in ipairs(FarmPage:GetChildren()) do
        child:Destroy()
    end

    if not SupportedLoaded then
        makeLabel(FarmPage, "Loading support status...", 10)
        return
    end

    if not IsGameSupported then
        makeLabel(FarmPage, "Game is NOT supported", 10)
        makeLabel(FarmPage, "Farm features are disabled.", 32)
        return
    end

    if CurrentGameId == 84968446824850 then
        makeLabel(FarmPage, "Brainrot Free Eats — Custom Farm", 6)

        makeButton(FarmPage, "Tp to end", 32, function()
        ...
        makeButton(FarmPage, "Upgrade all", 72, function()
            local Event = ReplicatedStorage:FindFirstChild("Remotes") and ReplicatedStorage.Remotes:FindFirstChild("UpgradeBrainrot")
            local plot = getPlayerPlot()
            local pods = plot and plot:FindFirstChild("Pods")
        ...
        makeButton(FarmPage, "Collect Cash", 112, function()
            collectAllCash()
        end)

        makeButton(FarmPage, "Back to base", 152, function()
            disableInstant()
            autoMoney = false
            autoUpgrade = false
            cachedPlot = nil
        ...
        makeToggleButton(FarmPage, "Auto Collect Money", 200, function(state)
            autoMoney = state
            if state then
                cachedPlot = nil
                startAutoMoney()
        ...
        makeToggleButton(FarmPage, "Auto Upgrade", 244, function(state)
            autoUpgrade = state
            if state then
                cachedPlot = nil
                startAutoUpgrade()
        ...
        makeLabel(FarmPage, "Supported game detected", 6)
        makeLabel(FarmPage, "No custom farm loaded for this PlaceId yet.", 28)
        makeLabel(FarmPage, "Send the script and I’ll wire it in.", 48)
    end
end
...
print("[xexy hub] loaded — supported games list + teleport ready")
